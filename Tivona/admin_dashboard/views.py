...
def admin_offers(request):
    ...
    for offer in category_offers:
        if offer.valid_to < now and offer.is_active:
            ...
            offer.save()
            messages.info(request, f'Category offer "{offer}" has expired and was deactivated.')
...
def add_category_offer(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if valid_from <= timezone.now() <= valid_to:
                category_offer.active = True
            else:
                category_offer.active = False
            ...
            for product in products:
                ProductOffer.objects.create(
                    ...
                    valid_from = valid_from,
                    valid_to = valid_to,
                    is_active=category_offer.is_active
                )
...
def add_product_offer(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if variant and ProductOffer.objects.filter(variant=variant).exists():
                messages.error(request, 'This variant already has an offer.')
                return redirect('admin_offers')
            elif not variant and ProductOffer.objects.filter(product=product, variant__isnull=True).exists():
                messages.error(request, 'This product already has an active offer without specifying a variant.')
                return redirect('admin_offers')
...
def get_variants(request, product_id):
    ...
    variants_names = [
        {'id': variant.id, 'color': get_color_name(variant.color)} for variant in variants
    ]
...
def  change_category_offer_status(request,offer_id):
    ...
    offered_products = ProductOffer.objects.filter(
        product__category=offer.category,
        discount_percentage=offer.discount_percentage,
        valid_from=offer.valid_from,
        valid_to=offer.valid_to,
    )
    for product_offer in offered_products:
        product_offer.is_active = offer.is_active
        product_offer.save()
